...
// 전역 변수
var (
	subscribe = map[int]chan (chan<- Subscription){}// 구독 채널
	unsubscribe = map[int]chan (<-chan Event){}		// 구독 해지 채널
	publish = map[int]chan Event{}					// 이벤트 발행 채널
...
func NewEvent(evtType, userId, msg string, chatroomNo int) Event {
	return Event{evtType, userId, int(time.Now().Unix()), msg, chatroomNo}
}

// 새로운 사용자가 들어왔을 때 이벤트를 구독할 함수
func Subscribe(chatroomNo int) Subscription {
	c := make(chan Subscription)	// 채널 생성
	subscribe[chatroomNo] <- c		// 구독 채널에 보냄
	return <-c
}

// 사용자가 나갔을 때 구독을 취소할 함수
func (s Subscription) Cancel(chatroomNo int) {
	unsubscribe[chatroomNo] <- s.New	// 구독 해지 채널에 보냄
...
// 사용자가 들어왔을 때 이벤트 발행
func Join(userName string, chatroomNo int) {
	// 사용자 이름으로 join 이벤트를 만들고 publish 채널로 보냄
	publish[chatroomNo] <- NewEvent("join", userName, "", chatroomNo)

	fmt.Println(strconv.Itoa(chatroomNo) + " : " + userName + " user joined")
}

// 사용자가 채팅 메시지를 보냈을 때 이벤트 발행
func Say(userName, message string, chatroomNo int) {
	// 사용자 이름, 메시지로 message 이벤트를 만들고 publish 채널로 보냄
	publish[chatroomNo] <- NewEvent("message", userName, message, chatroomNo)

	fmt.Println(strconv.Itoa(chatroomNo) + " : " + userName + " user said " + message)
}

// 사용자가 나갔을 때 이벤트 발행
func Leave(userName string, chatroomNo int) {
	// 사용자 이름으로 leave 이벤트를 만들고 publish 채널로 보냄
	publish[chatroomNo] <- NewEvent("leave", userName, "", chatroomNo)

	fmt.Println(strconv.Itoa(chatroomNo) + " : " + userName + " user left")
}
	...
	for {	// 무한 루프
		...
		case newUser := <-login:
			...
			if found {
				...
				// 유저에게 로그인 성공 메시지를 보냄
				newUser.UserChan <- NewEvent("loginsuccess", newUser.UserName, "", 0)
			} else {
				fmt.Println(newUser.UserId + " " + newUser.UserName + " user failed")
				// 유저에게 로그인 실패 메시지를 보냄
				newUser.UserChan <- NewEvent("loginfailed", newUser.UserName, "", 0)
			}
...
// 채팅방 처리
func Chatroom(chatroomNo int) {

	if _, ok := subscribe[chatroomNo]; !ok {
		// 채널들 초기화
		subscribe[chatroomNo] = make(chan (chan<- Subscription), 10)
		unsubscribe[chatroomNo] = make(chan (<-chan Event), 10)
		publish[chatroomNo] = make(chan Event, 10)

		fmt.Println(strconv.Itoa(chatroomNo) + " chat room created")

		chatroomCount++
	} else {
		// 이미 생성되어있는 채팅방이면 고루틴 종료
		return
	}
	...
	for {	// 무한 루프
		select {
		case c := <-subscribe[chatroomNo]:	// 새로운 사용자가 들어왔을 때 -> subscribe에 값이 들어옴
			...
			// 구독 구조체 인스턴스 생성하고 채널 c로 보냄
			c <- Subscription{events, subscriber, chatroomNo}

		case event := <-publish[chatroomNo]:	// 새 이벤트가 발행되었을 때 -> publish에 값이 들어옴 
									// Join, Say, Leave
			// 모든 사용자에게 이벤트 전달
		...
		case c := <-unsubscribe[chatroomNo]:	// 사용자가 나갔을 때 -> unsubscribe에 값이 들어옴 
			for e := subscribers.Front(); e != nil; e = e.Next() {
				// 구독자 목록에서 이벤트 채널을 꺼냄
				subscriber := e.Value.(chan Event)
...
func main() {
	...
	go Chatroom(0) // 채팅방 처리 함수를 고루틴으로 실행
	chatroomCount--// 이 방은 채팅방이 아니고 통신 처리용
	...
	server.On("connection", func(so socketio.Socket) {
		...
		// 웹 브라우저에서 보내오는 채팅 방 들어가는 요청
		so.On("chatroom", func(chatroomStr string) {
			
			Leave(GetUserName(so.Id()), s.ChatroomNo)
			// 구독 취소
			s.Cancel(s.ChatroomNo)

			chatroomNo, _ := strconv.Atoi(chatroomStr)

			// 새 채팅방 생성
			go Chatroom(chatroomNo)
			time.Sleep(100 * time.Millisecond)
			// 새 채팅방 구독
			s = Subscribe(chatroomNo)	// 구독 처리
			s.ChatroomNo = chatroomNo
			...
			// 사용자가 채팅방에 들어왔다는 이벤트 발행
			Join(GetUserName(so.Id()), chatroomNo)

			// 현재 채팅방 개수 알려줌
			for _, v := range publish {
				v <- NewEvent("chatroomno", "", "", chatroomCount)
			}		
		})
		...
		go func() {
			for {	 // 무한 루프
				...
				case msg := <-newMessages:	// 웹 브라우저에서 채팅 메시지를 보내오면
					// 채팅 메시지 이벤트 발행
					Say(GetUserName(so.Id()), msg, s.ChatroomNo)
